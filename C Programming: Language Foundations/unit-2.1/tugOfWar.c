...
int main(){
    int totalTeamMembers,team1,team2,i,sum1,sum2;
    sum1 = 0;
    sum2 = 0;
    scanf("%d", &totalTeamMembers);
    for(i=0; i<totalTeamMembers; i++){
        scanf("%d", &team1);
        sum1 = sum1+team1;
        scanf("%d", &team2);
        sum2 = sum2+team2;
    ...
    if (sum1>sum2){
        printf("Team %d has an advantage\nTotal weight for team %d: %d\nTotal weight for team %d: %d", 1, 1, sum1, 2, sum2);
    }else{
    if(sum2>sum1){
        printf("Team %d has an advantage\nTotal weight for team %d: %d\nTotal weight for team %d: %d", 2, 1, sum1, 2, sum2);
    }
    }
    return 0;
}